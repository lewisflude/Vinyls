...
class Lastfm
  ...
  def fetch_album_genre(artist, title)
    ...
    escaped_title = URI::escape(title)
    lastfm_key = ENV["LASTFM_KEY"]
    album = RestClient.get("http://ws.audioscrobbler.com/2.0/?method=album.getinfo&artist=#{escaped_artist}&album=#{escaped_title}&api_key=#{lastfm_key}&format=json")
    results = JSON.parse(album).fetch('album').fetch('toptags').fetch('tag')[0].fetch('name')
  end
end
